...
import type { CarrinhoBody, CarrinhoQuery, CarrinhoRouteParams, CarrinhoUpdateData } from "../types/carrinho.types";
...
class CarrinhoPayload {
  ...
  get userId() { return CarrinhosController.parsePositiveId(this.body.id_usuario); }
...
class CarrinhosController {
  ...
  private static getCreateErrorMessage(payload: CarrinhoPayload) {
    if (!payload.hasUserField()) return "id_usuario e obrigatorio.";
    if (!payload.userId) return "id_usuario invalido.";
    return null;
  }

  private static getUpdateErrorMessage(payload: CarrinhoPayload) {
    return payload.hasUserField() && !payload.userId ? "id_usuario invalido." : null;
  }
  ...
  private static buildUpdateData(carrinho: Carrinhos, payload: CarrinhoPayload): CarrinhoUpdateData {
    return { id_usuario: payload.userId ?? carrinho.id_usuario };
  }

  static async findAll(req: CarrinhoRequest, res: Response) {
    const userId = req.query.id_usuario === undefined ? undefined : CarrinhosController.parsePositiveId(req.query.id_usuario);
    if (req.query.id_usuario !== undefined && !userId) return res.status(400).json({ message: "id_usuario invalido." });
    return res.status(200).send(await Carrinhos.findAll({ where: userId ? { id_usuario: userId } : {} }));
  }
  ...
  static async getById(req: CarrinhoRequest, res: Response) {
    ...
    if (!carrinho) return res.status(404).json({ message: "Carrinho nao encontrado." });
    return res.status(200).send(carrinho);
  }

  static async create(req: CarrinhoRequest, res: Response) {
    const payload = new CarrinhoPayload(req.body), message = CarrinhosController.getCreateErrorMessage(payload);
  ...
  static async update(req: CarrinhoRequest, res: Response) {
    const cartId = CarrinhosController.parsePositiveId(req.params.id), payload = new CarrinhoPayload(req.body);
    if (!cartId) return res.status(400).json({ message: "ID do carrinho invalido." });
    const carrinho = await Carrinhos.findByPk(cartId), message = CarrinhosController.getUpdateErrorMessage(payload);
    if (!carrinho) return res.status(404).json({ message: "Carrinho nao encontrado." });
    if (message) return res.status(400).json({ message });
    const userMessage = await CarrinhosController.findMissingUserMessage(payload.userId);
    if (userMessage) return res.status(404).json({ message: userMessage });
