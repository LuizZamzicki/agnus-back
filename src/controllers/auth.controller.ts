...
class AuthController {
    ...
    if (!authResult) {
      ...
      if (senhaJaUsada != null) {
        ...
        return res.status(401).json({
          message: `Essa senha já foi usada ${elapsedTime} (em ${dataFormatada}).`,
        });
      }
  ...
  static async googleCallback(req: Request, res: Response) {
    try {
      ...
      if (error) {
        return res.status(400).json({ message: `Google OAuth retornou erro: ${String(error)}` });
      }

      if (!code || !state || typeof code !== "string" || typeof state !== "string") {
        return res.status(400).json({ message: "Parametros OAuth inválidos." });
      }
      ...
      return res.status(401).json({
        message: error instanceof Error ? error.message : "Falha no callback do Google OAuth.",
      });
    }
  }
}
