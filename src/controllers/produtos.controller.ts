...
import type { ProdutoBody, ProdutoQuery, ProdutoRouteParams } from "../types/produto.types";
...
class ProdutosController {
  ...
  private static sendResult(res: Response, result: { message: string; status?: number } | { data: object; pagination?: object }, successStatus = 200) {
    if ("message" in result) return res.status(result.status ?? 400).json({ message: result.message });
    return result.pagination ? res.status(successStatus).json(result) : res.status(successStatus).send(result.data);
  }
  ...
  static async create(req: ProdutoRequest, res: Response) {
    ...
    if ("message" in result) return res.status(result.status ?? 400).json({ message: result.message });
    return res.status(201).json({ ...result.data.produto, grades: result.data.grades, cores: result.data.cores, fotos: result.data.fotos });
  }
  ...
  static async remove(req: ProdutoRequest, res: Response) {
    ...
    return "message" in result ? res.status(result.status ?? 400).json({ message: result.message }) : res.status(204).send();
  }
}
