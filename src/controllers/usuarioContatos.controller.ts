...
class UsuarioContatoPayload {
  ...
  get tipo() { return UsuarioContatosController.normalizeTipo(this.body.tipo); }
  get valor() { return this.body.valor?.trim() || null; }
  ...
  hasValorField() { return this.body.valor !== undefined; }
...
class UsuarioContatosController {
  ...
  static normalizeTipo(value: UsuarioContatoBody["tipo"]) {
    return typeof value === "string" && UsuarioContatosController.VALID_TYPES.includes(value as UsuarioContatoTipo) ? value as UsuarioContatoTipo : null;
  }

  private static getCreateErrorMessage(payload: UsuarioContatoPayload) {
    if (!payload.hasUserField() || !payload.hasValorField()) return "id_usuario e valor sao obrigatorios.";
    if (!payload.userId) return "id_usuario invalido.";
    if (!payload.valor) return "valor invalido.";
    if (payload.hasTipoField() && !payload.tipo) return "tipo invalido.";
    return null;
  }
  ...
  private static getUpdateErrorMessage(payload: UsuarioContatoPayload) {
    ...
    if (payload.hasValorField() && !payload.valor) return "valor invalido.";
    if (payload.hasTipoField() && !payload.tipo) return "tipo invalido.";
    return null;
  }
  ...
  private static buildUpdateData(contato: UsuarioContatos, payload: UsuarioContatoPayload): UsuarioContatoUpdateData {
    return { id_usuario: payload.userId ?? contato.id_usuario, tipo: payload.tipo ?? contato.tipo, valor: payload.valor ?? contato.valor, principal: payload.hasPrincipalField() ? payload.principal : contato.principal };
  }
  ...
  static async getByIdUser(req: UsuarioContatoRequest, res: Response) {
    ...
    if (!contatos) return res.status(404).json({ message: "Contato nao encontrado." });
    return res.status(200).send(contatos);
  }

  static async create(req: UsuarioContatoRequest, res: Response) {
    const payload = new UsuarioContatoPayload(req.body), message = UsuarioContatosController.getCreateErrorMessage(payload);
    ...
    return res.status(201).send(await UsuarioContatos.create({ id_usuario: payload.userId!, tipo: payload.tipo ?? "celular", valor: payload.valor!, principal: payload.principal }));
  }

  static async update(req: UsuarioContatoRequest, res: Response) {
    const contactId = UsuarioContatosController.parsePositiveId(req.params.id), payload = new UsuarioContatoPayload(req.body);
    if (!contactId) return res.status(400).json({ message: "ID do contato invalido." });
    const contato = await UsuarioContatos.findByPk(contactId), message = UsuarioContatosController.getUpdateErrorMessage(payload);
    if (!contato) return res.status(404).json({ message: "Contato nao encontrado." });
    if (message) return res.status(400).json({ message });
    const userMessage = await UsuarioContatosController.findUserError(payload.userId);
    if (userMessage) return res.status(404).json({ message: userMessage });
