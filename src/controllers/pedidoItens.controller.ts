...
class PedidoItemPayload {
  ...
  get orderId() { return this.parseId(this.body.id_pedido); }
...
class PedidoItensController {
  ...
  private static getCreateErrorMessage(payload: PedidoItemPayload) {
    if (!payload.hasOrderField() || !payload.hasColorField() || !payload.hasGradeField() || !payload.hasQuantityField()) return "id_pedido, id_produto_cor, id_produto_grade e quantidade sao obrigatorios.";
    if (!payload.orderId) return "id_pedido invalido.";
  ...
  private static getUpdateErrorMessage(payload: PedidoItemPayload) {
    if (payload.hasOrderField() && !payload.orderId) return "id_pedido invalido.";
  ...
  private static async findOrderError(payload: PedidoItemPayload) {
    return payload.hasOrderField() && !(await Pedidos.findByPk(payload.orderId!)) ? "Pedido nao encontrado." : null;
  }
  ...
  private static async resolvePricing(selection: PedidoItemSelection, quantidade: number): Promise<PedidoItemPricing | null> {
    ...
    return produtoContext ? { preco_unitario: produtoContext.precoUnitario, subtotal: calculateSubtotal(produtoContext.precoUnitario, quantidade) } : null;
  }
  ...
  private static buildCreateData(payload: PedidoItemPayload, pricing: PedidoItemPricing): PedidoItemMutationData {
    return { id_pedido: payload.orderId!, id_produto_cor: payload.colorId!, id_produto_grade: payload.gradeId!, quantidade: payload.quantidade, preco_unitario: pricing.preco_unitario, subtotal: pricing.subtotal };
  }

  private static buildUpdateData(item: PedidoItens, payload: PedidoItemPayload, selection: PedidoItemSelection, pricing: PedidoItemPricing): PedidoItemMutationData {
    return { id_pedido: payload.orderId ?? item.id_pedido, id_produto_cor: selection.colorId, id_produto_grade: selection.gradeId, quantidade: payload.hasQuantityField() ? payload.quantidade : item.quantidade, preco_unitario: pricing.preco_unitario, subtotal: pricing.subtotal };
  }
  ...
  static async create(req: PedidoItemRequest, res: Response) {
    const payload = new PedidoItemPayload(req.body), message = PedidoItensController.getCreateErrorMessage(payload);
    if (message) return res.status(400).json({ message });
    const orderMessage = await PedidoItensController.findOrderError(payload), selection = PedidoItensController.buildSelection(null, payload), selectionError = await PedidoItensController.resolveSelectionError(selection), pricing = selectionError ? null : await PedidoItensController.resolvePricing(selection, payload.quantidade);
    if (orderMessage) return res.status(404).json({ message: orderMessage });
    if (selectionError) return res.status(selectionError.status).json({ message: selectionError.message });
    if (!pricing) return res.status(404).json({ message: "Produto vinculado ao item nao encontrado." });
    return res.status(201).send(await PedidoItensController.enrichItem(await PedidoItens.create(PedidoItensController.buildCreateData(payload, pricing))));
  }

  static async update(req: PedidoItemRequest, res: Response) {
    const itemId = PedidoItensController.parsePositiveId(req.params.id), payload = new PedidoItemPayload(req.body);
    if (!itemId) return res.status(400).json({ message: "ID do item invalido." });
    const item = await PedidoItens.findByPk(itemId), message = PedidoItensController.getUpdateErrorMessage(payload);
    if (!item) return res.status(404).json({ message: "Item do pedido nao encontrado." });
    if (message) return res.status(400).json({ message });
    const orderMessage = await PedidoItensController.findOrderError(payload), selection = PedidoItensController.buildSelection(item, payload), selectionError = await PedidoItensController.resolveSelectionError(selection), pricing = selectionError ? null : await PedidoItensController.resolvePricing(selection, payload.hasQuantityField() ? payload.quantidade : item.quantidade);
    if (orderMessage) return res.status(404).json({ message: orderMessage });
    if (selectionError) return res.status(selectionError.status).json({ message: selectionError.message });
    if (!pricing) return res.status(404).json({ message: "Produto vinculado ao item nao encontrado." });
    await item.update(PedidoItensController.buildUpdateData(item, payload, selection, pricing));
