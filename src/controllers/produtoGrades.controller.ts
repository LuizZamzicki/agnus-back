...
import type { ProdutoGradeBody, ProdutoGradeRouteParams, ProdutoGradeUpdateData } from "../types/produto-grade.types";
...
class ProdutoGradesController {
  ...
  private static getCreateErrorMessage(payload: ProdutoGradePayload) {
    if (!payload.hasProductField() || !payload.hasNameField()) return "id_produto e nome sao obrigatorios.";
  ...
  private static buildUpdateData(grade: ProdutoGrades, payload: ProdutoGradePayload): ProdutoGradeUpdateData {
    return { id_produto: payload.productId ?? grade.id_produto, nome: payload.nome ?? grade.nome, acrescimo: payload.hasAcrescimoField() ? payload.acrescimo : grade.acrescimo };
  }
  ...
  static async getByIdProduto(req: ProdutoGradeRequest, res: Response) {
    ...
    if (!grades) return res.status(404).json({ message: "Grade do produto nao encontrada." });
    return res.status(200).send(grades);
  }

  static async create(req: ProdutoGradeRequest, res: Response) {
    const payload = new ProdutoGradePayload(req.body), message = ProdutoGradesController.getCreateErrorMessage(payload);
    ...
    return res.status(201).send(await ProdutoGrades.create({ id_produto: payload.productId!, nome: payload.nome!, acrescimo: payload.acrescimo }));
  }

  static async update(req: ProdutoGradeRequest, res: Response) {
    const gradeId = ProdutoGradesController.parsePositiveId(req.params.id), payload = new ProdutoGradePayload(req.body);
    if (!gradeId) return res.status(400).json({ message: "ID da grade invalido." });
    const grade = await ProdutoGrades.findByPk(gradeId), message = ProdutoGradesController.getUpdateErrorMessage(payload);
    if (!grade) return res.status(404).json({ message: "Grade do produto nao encontrada." });
    if (message) return res.status(400).json({ message });
    const productMessage = await ProdutoGradesController.findProductError(payload.productId);
    if (productMessage) return res.status(404).json({ message: productMessage });
